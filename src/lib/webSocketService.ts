...
import { joinedGameCallback, roomEnded, updateState, getPlaying, reJoinRoom, getName } from './gameService';
...
export function websocketSetup() {
    ...
    // prefer binary frames for protobuf Envelopes
...
function toClientMessageFromGeneric(message: any, requestId?: string) {
    ...
    switch (type) {
        case 'joinRoom':
            cm.joinRoom = { roomId: data.roomId || data.roomID || data.room || '', name: data.name || '', userID: data.userID || data.userId || '' };
            break;
        case 'getTime':
            cm.getTime = {};
            break;
        case 'ping':
            ...
            break;
        case 'sendMessage':
            cm.sendMessage = { clientName: data.clientName || getName(), message: typeof data === 'string' ? data : JSON.stringify(data) };
            break;
        case 'gameState':
        ...
        default:
            ...
            if (message.joinRoom || message.getTime || message.ping || message.sendMessage || message.gameState) {
                Object.assign(cm, message);
            } else {
                // fallback: send as low-latency/raw string inside lowLatency
                cm.lowLatency = { payload: new TextEncoder().encode(JSON.stringify(message)) };
            }
    }
