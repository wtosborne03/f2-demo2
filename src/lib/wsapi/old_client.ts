...
export class GameClient {
    ...
    this.ws.onopen = () => {
      ...
      if (this.autoRejoin && this.role === "PLAYER" && this.lastRoomCode && this.lastPlayerName) {
        try {
          ...
          setTimeout(() => this.joinRoom(this.lastRoomCode!, this.lastPlayerName!, this.lastUserId || ""), 50);
        } catch (e) {
          /* ignore */
        }
      }
  ...
  private send(packet: WsClientPacket) {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
  ...
  setStage(stage: GameStage) {
    if (!this.lastRoomCode || !this.hostToken) {
      throw new Error("setStage requires room code and host token");
    }
    this.send({
      packet: {
        $case: "setStage",
  ...
  joinRoom(roomCode: string, playerName: string, userId: string) {
    ...
    this.send({
      packet: { $case: "joinRoom", joinRoom: { roomCode, playerName, userId } },
    });
