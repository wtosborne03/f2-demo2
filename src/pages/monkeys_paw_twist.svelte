...
>
  ...
  >
    ...
    <div class="flex items-center justify-center gap-2">
      ...
      <span class="text-xl font-black text-[#fca5a5] uppercase tracking-wider"
        >Submit a Twist</span
      >
    </div>
    ...
    <div class=" rounded-xl p-3 text-center">
      <div class="text-md font-bold text-[#fca5a5] uppercase mb-1">
        {m_data?.wishMaker || "Player"}'s Wish
      </div>
      <div class="text-lg font-extrabold text-white italic">
    ...
    <form
      class="grow flex flex-col w-full h-36"
      on:submit|preventDefault={submit_prompt}
    >
      <textarea
        class="w-full h-full bg-[#0c0107]/90 rounded-xl p-3 resize-none outline-none text-base text-white placeholder-red-300/40 text-center flex items-center justify-center leading-relaxed focus:border-[#f87171] transition-colors"
        placeholder={m_data?.placeholder ||
          "...but it is entirely in loose pennies!"}
