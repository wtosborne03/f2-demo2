<script lang="ts">
  ...
  $: triviaData =
    rawPageData?.trivia || (rawPageData?.question ? rawPageData : null);
  ...
  $: currentQuestionKey = triviaData
    ? triviaData.questionId || triviaData.question || "trivia"
    : null;
...
>
  <!-- Header Bar -->
  <header
    class="flex items-center justify-between border-b border-base-content/10 pb-2 shrink-0 z-30"
  >
    ...
    {#if triviaData}
      <div
        class="badge badge-warning font-mono text-[11px] font-bold py-1.5 px-2.5 animate-pulse"
      >
        TRIVIA
      </div>
    {/if}
  ...
  {#if triviaData}
    ...
    <div
      class="flex-1 flex flex-col justify-between min-h-0 py-2 relative z-20 gap-2"
    >
      <!-- TOP TRIVIA CARD -->
      <div
        class="flex-initial w-full flex flex-col justify-between p-3.5 rounded-2xl bg-base-200/95 border border-warning/40 shadow-xl backdrop-blur-md relative overflow-visible shrink-0"
      >
          ...
          >
            ...
            {:else}
              ...
              {#if triviaData.correctOption}
                <div
                  class="mt-1 px-2.5 py-1 bg-base-100/80 rounded-lg text-xs font-mono font-bold text-error border border-error/30"
                >
        ...
        {:else}
          ...
          <div class="shrink-0 mb-1.5">
            <div class="flex items-center justify-between gap-2 mb-1">
              <span
                class="text-[11px] font-mono font-bold text-warning uppercase"
              >
                TRIVIA QUESTION
              </span>
              <span
                class="badge badge-warning badge-sm font-mono font-black text-[11px] px-2 py-0.5"
              >
          ...
          <!-- Question Prompt -->
          <div
            class="bg-base-300/80 rounded-xl p-2.5 text-center shrink-0 shadow-inner mb-2"
          >
            ...
            {#each questionOptions as option, idx}
              ...
              >
                <div class="flex items-center gap-1.5 min-w-0 flex-1">
                  <span
                    class={`badge badge-xs font-mono font-bold shrink-0 ${isSelected ? "badge-primary-content text-primary" : "badge-ghost"}`}
                  >
                    {optionLetters[idx]}
                  </span>
                  <span
                    class="leading-tight text-[11px] sm:text-xs font-medium break-words line-clamp-2"
                    >{option}</span
                  >
                </div>
              </button>
      ...
      >
        ...
        <!-- Center Indicator -->
        <div
          class="flex items-center gap-2 pointer-events-none select-none opacity-60"
        >
          <span class="text-sm font-bold text-primary">▲</span>
          <span
            class="text-xs font-mono font-bold uppercase tracking-wider text-base-content/80"
          >
            SWIPE TO STEER
          </span>
          <span class="text-sm font-bold text-primary">▼</span>
  ...
  {:else}
    ...
    >
      ...
      <div
        class="flex flex-col items-center gap-2 opacity-60 pointer-events-none select-none"
      >
        <div
          class="w-12 h-12 rounded-full border border-base-content/20 flex items-center justify-center bg-base-100/40 shadow-sm"
        >
          <svg
        ...
        <span
          class="text-xs font-mono font-bold uppercase tracking-widest text-base-content/70"
        >
          SWIPE TO STEER
        </span>
      </div>
