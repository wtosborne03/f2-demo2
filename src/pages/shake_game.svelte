<script lang="ts">
  ...
  function trySend() {
    ...
    if (now - lastSendTime > 7) {
      gameClient.sendInput({
        type: "shakeProgress", // Ensure this exists in your protobuf/types
...
<div class="container h-full mx-auto flex flex-col justify-center items-center">
  {#if motion}
    <div class="border-4 p-3" style:border-color={get(gameState).color}>
    ...
    <div class="flex flex-col items-center gap-4">
      ...
      >
        ...
        >
          <g>
            <path
              style:fill={`${get(gameState).color}`}
              class="st0"
